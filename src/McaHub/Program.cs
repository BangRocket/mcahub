...
if (StartupGuard.PublicExposureViolation(auth.Accounts, auth.HasMaster ? "set" : null, auth.DevLogin, bindUrls, allowPublicOpen) is { } refusal)
    throw new InvalidOperationException($"mcahub refusing to start: {refusal}");
...
app.Logger.LogInformation("mcahub serving worlds from {DataDir} · auth: {Mode}", Path.GetFullPath(dataDir), mode);
