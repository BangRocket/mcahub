...
public static class Transport
{
    ...
    public static void MapTransport(WebApplication app, RepoStore store, HubDb db, Auth.Config cfg, long maxBody, AuthThrottle throttle, bool adoptUnowned, AuditLog audit, bool defaultPrivate, int maxWorldsPerUser, string? discordWebhook = null)
    {
        ...
        app.MapPost("/r/{repo}/objects/{hash}", async (string repo, string hash, HttpRequest req, HttpContext ctx) =>
            await Write(store, db, cfg, repo, ctx, throttle, adoptUnowned, audit, defaultPrivate, maxWorldsPerUser, async (s, _) => s.PutObject(hash, await Bytes(req, maxBody))));

        // Upload a whole pack (the common push path).
        app.MapPost("/r/{repo}/pack", async (string repo, HttpRequest req, HttpContext ctx) =>
            await Write(store, db, cfg, repo, ctx, throttle, adoptUnowned, audit, defaultPrivate, maxWorldsPerUser, async (s, _) =>
            {
        ...
        app.MapPost("/r/{repo}/refs/heads/{branch}", async (string repo, string branch, HttpRequest req, HttpContext ctx) =>
            await Write(store, db, cfg, repo, ctx, throttle, adoptUnowned, audit, defaultPrivate, maxWorldsPerUser, async (s, actor) =>
            {
    ...
    private static async Task<IResult> Write(RepoStore store, HubDb db, Auth.Config cfg, string repo, HttpContext ctx,
        AuthThrottle throttle, bool adoptUnowned, AuditLog audit, bool defaultPrivate, int maxWorldsPerUser, Func<RemoteService, string, Task> body)
    {
        ...
        if (created && cfg.Accounts && !admin && uid is not null && maxWorldsPerUser > 0 && db.OwnedRepoCount(uid) >= maxWorldsPerUser)
            // fair-use governance cap (#35), distinct from the per-IP/size DoS bounds — stops one account flooding the hub
        ...
        {
            ctx.RequestServices.GetRequiredService<ILoggerFactory>().CreateLogger("McaHub.Transport")
